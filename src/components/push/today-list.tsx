...
export function TodayList({ rows, loading, onSelect, onSent, onSkip }: Props) {
  ...
  return (
    <>
      {/* Column headers — visible on md+ to give the date columns context */}
      <div className="hidden md:flex items-center gap-3 px-4 py-2 text-xs font-medium uppercase tracking-wide text-gray-500">
        <div className="shrink-0 w-16">Pri</div>
        ...
        {rows.map((r) => {
          ...
          return (
            ...
            >
              {/* Priority pill */}
              <div
                className={`shrink-0 w-16 rounded-md border px-2 py-1 text-xs font-semibold text-center ${PRIORITY_BADGE[r.priority] || PRIORITY_BADGE.STANDARD}`}
                title="Priority"
              >
              ...
              {/* Next touch */}
              <div className="hidden md:flex flex-col shrink-0 w-32">
                <div className="text-sm font-medium text-gray-900">{touchLabel(r.currentTouch)}</div>
                <div className="text-xs text-gray-500">touch {r.currentTouch}/6 done</div>
              </div>

              {/* Last contact */}
              <div className="hidden md:flex flex-col shrink-0 w-32">
                <div className="text-sm font-medium text-gray-900">
                  {shortDate(r.lastTouchAt)}
                </div>
                <div className="text-xs text-gray-500">
                  {r.lastTouchAt ? pastRelative(r.lastTouchAt) : "no outbound yet"}
                </div>
              </div>
              ...
              <div className="hidden md:flex flex-col shrink-0 w-32">
                <div className="text-sm font-medium text-gray-900">
                  {shortDate(r.nextTouchDueAt)}
                </div>
                <div className={`text-xs ${dueRelativeColor(r.nextTouchDueAt)}`}>
                  {dueRelative(r.nextTouchDueAt)}
                </div>
              </div>

              {/* Actions */}
              <div className="flex gap-1 shrink-0 w-44 justify-end">
                <Button
                  size="sm"
